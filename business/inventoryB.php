...
<?php
    ...
    //echo $test->MarkOfSoldItiems(1, $from, $to,"2019-09-03");
    ...
    class InventoryB{
        ...
        public function GetRelevantProductID($from, $to){
            $FROM = "'" . $from . "'";
            $TO = "'" . $to . "'";
            $sql = "SELECT DISTINCT product_id FROM inventory_performance WHERE form_date>{$FROM} AND to_date<{$TO}";
            $db = new Database();
            $result = $db->select($sql);
            return $result;
        }
        ...
        public function UpdatePerformanceTable($product_id, $from, $to){
            ...
            $sql = "INSERT INTO `inventory_performance`(`product_id`, `form_date`, `to_date`, `performance`) VALUES ({$product_id},{$FROM},{$TO},{$performance})";
            $db = new Database();
            $db->insert($sql);
        }
        //from 1/9 -> 5/9
        public function CalculatePerformance($product_id, $from, $to){
            ...
            while ($row = mysqli_fetch_array($list)){
                ...
                $sum_M_S += $this->MarkOfSoldItiems($inventory_id, $from, $to, $import_date);
        ...
        public function MarkOfSoldItiems($inventory_id, $from, $to, $import_date){
            //1. Get correct row
